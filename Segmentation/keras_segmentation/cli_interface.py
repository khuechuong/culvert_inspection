...
from .train import train, train_continue
from .predict import *
...
def train_action(command_parser):
    ...
    parser.add_argument("--do_augment", type=bool, default=False)
...
def train_continue_action(command_parser):
    parser = command_parser.add_parser('train_continue')
    parser.add_argument("--model_name", type=str, required=True)
    parser.add_argument("--train_images", type=str, required=True)
    parser.add_argument("--train_annotations", type=str, required=True)
    ...
    def action(args):
        return train_continue(model=args.model_name,
                     train_images=args.train_images,
                     train_annotations=args.train_annotations,
                     input_height=args.input_height,
                     input_width=args.input_width,
...
def predict_test_action(command_parser):

    parser = command_parser.add_parser('predict_test')
    parser.add_argument("--checkpoints_path", type=str, required=True)
    parser.add_argument("--input_path", type=str, default="", required=True)
    parser.add_argument("--output_path", type=str, default="", required=True)
    ...
    def action(args):
        input_path_extension = args.input_path.split('.')[-1]
        if input_path_extension in ['jpg', 'jpeg', 'png']:
            return predict_test(inp=args.input_path, out_fname=args.output_path,
                           checkpoints_path=args.checkpoints_path)
        else:
            return predict_multiple_test(inp_dir=args.input_path,
                                    out_dir=args.output_path,
                                    checkpoints_path=args.checkpoints_path)
...
def evaluate_model_test_action(command_parser):

    parser = command_parser.add_parser('evaluate_model_test')
    parser.add_argument("--images_path", type=str, required=True)
    parser.add_argument("--segs_path", type=str, required=True)
    parser.add_argument("--checkpoints_path", type=str, required=True)
...
def best_model_test_action(command_parser):

    parser = command_parser.add_parser('best_model_test')
    parser.add_argument("--images_path", type=str, required=True)
    parser.add_argument("--segs_path", type=str, required=True)
    parser.add_argument("--checkpoints_path", type=str, required=True)
...
def main():
    ...
    train_continue_action(command_parser)
    predict_action(command_parser)
    predict_test_action(command_parser)
    ...
    evaluate_model_test_action(command_parser)
    best_model_test_action(command_parser)
